 ...
 *  단, 조인할 때 기분이 되는 컬럼명이 같아야 한다.
 */
SELECT ONO, T1.PNO, T1.QTY,PNAME
FROM ORDERS T1 JOIN PRODUCTS T2 USING(PNO);

/* 2)NATURAL 조인 => T1 JOIN T2 ON 조인조건 1
...
/* 문제: 다음 결과를 출력하시오!
 * 주문번호, 제품명, 제조사명, 주문 수량, 잔여 수량, 고객명, 고객이메일 * 
 * 
 * */

SELECT ONO, PNAME, MKNAME, T1.QTY, T2.QTY, UNAME, EMAIL
FROM ORDERS T1 JOIN PRODUCTS T2 JOIN MAKERS T3 JOIN MEMBERS T4
WHERE T4.UID=T1.UID AND T1.PNO=T2.PNO AND T2.MKNO=T3.MKNO;
